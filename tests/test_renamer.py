...
def _renamed(out_dir: Path) -> Path:
    return out_dir / "Renommés"


def _originaux(out_dir: Path) -> Path:
    return out_dir / "Originaux"
...
def test_copie_renomme_sans_toucher_loriginal(tmp_path):
    ...
    assert (_renamed(out_dir) / "VD 1 JKT LEROY_TQG.jpeg").exists()
...
def test_collision_dans_le_meme_lot(tmp_path):
    ...
    names = sorted(p.name for p in _renamed(out_dir).iterdir())
...
def test_undo_supprime_les_copies(tmp_path):
    ...
    r = Renamer()
    r.execute("REF", "S", out_dir, [a])
    assert (_renamed(out_dir) / "REF_S.jpg").exists()
    assert r.can_undo()

    result = r.undo_last()
    assert result.removed == 1
    assert not (_renamed(out_dir) / "REF_S.jpg").exists()
...
def test_move_originals_et_undo(tmp_path):
    src_dir = tmp_path / "src"
    out_dir = tmp_path / "out"
    src_dir.mkdir()
    a = _make_image(src_dir, "a.jpg")
    ...
    r = Renamer()
    r.execute("REF", "S", out_dir, [a], move_originals=True)

    # L'original part dans out/Originaux ; la copie renommée dans out/Renommés.
    assert not a.exists()
    assert (_originaux(out_dir) / "a.jpg").exists()
    assert (_renamed(out_dir) / "REF_S.jpg").exists()

    # Annulation : copie supprimée ET original remis à sa place d'origine.
    result = r.undo_last()
    assert not result.failed
    assert not (_renamed(out_dir) / "REF_S.jpg").exists()
    assert a.exists()
    assert not (_originaux(out_dir) / "a.jpg").exists()
